...
function Router(options) {
    ...
    this.publish = function(topicUri, publicationId, publishArgs) {
	 _trace("Publish " + topicUri + " " + publicationId);
	 
	this.emit('Publish', [topicUri], publishArgs)
	if (typeof _topics[topicUri] !== 'undefined') {
	    for(var key in _topics[topicUri]) {
	        if(typeof _topics[topicUri][key] !== 'undefined') {
	            _topics[topicUri][key].apply(this, [publicationId, publishArgs]);
	        }
	    }
	    return true;
       } else {
