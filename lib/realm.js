...
function Api(router, realm) {
    ...
    this.yield = function (callId, err, args) {
...
function Realm(router) {
    ...
    // RPC Management
    this.regrpc = function(session, invID, procUri, options) {
        if (_rpcs.hasOwnProperty(procUri)) {
            session.sendError(WAMP.REGISTER, invID, "wamp.error.procedure_already_exists");
            return false;
        }
        ...
        router.emit('RPCRegistered', procUri, this);
        session.send([
            WAMP.REGISTERED,
            invID,
            registrationId
    ...
    this.unregrpc = function(session, requestId, regId) {
        ...
        if (_sessRPC.hasOwnProperty(session.sessionId) && _sessRPC[session.sessionId].hasOwnProperty(regId)) {
            ...
            router.emit('RPCUnRegistered', procUri, this);
            session.send([
                WAMP.UNREGISTERED,
                requestId
            ]);
    ...
    this.callrpc = function(session, callId, procUri, args, kwargs) {
        ...
        if (destSession) {
            ...
            log.trace('Invoking RPC ' + procUri, args, kwargs);
            destSession.invoke(_rpcs[procUri].regId, invId, args, kwargs);
            return invId;
        } else {
          // delete
        }
    ...
    this.resrpc = function(session, invId, err, args) {
        if (_pending.hasOwnProperty(invId)) {
            ...
            if (destSession)
                destSession.yield(_pending[invId][0], err, args);
        }
    ...
    this.substopic = function(session, requestId, topicUri, options) {
        ...
        router.emit('Subscribed', topicUri, this);
        session.send([
            WAMP.SUBSCRIBED,
            requestId,
            topicId
    ...
    this.unsubstopic = function(session, requestId, topicId) {
        ...
        if (_sessTopic.hasOwnProperty(session.sessionId) && _sessTopic[session.sessionId].hasOwnProperty(topicId)) {
            ...
            router.emit('UnSubscribed', topicUri, this);
            session.send([
                WAMP.UNSUBSCRIBED,
                requestId
            ]);
    ...
    this.publish = function(session, requestId, topicUri, options, args, kwargs) {
        ...
        if (_topics.hasOwnProperty(topicUri)) {
            for(var subscriptionId in _topics[topicUri]) {
                ...
                if (destSession) {
                    destSession.event(parseInt(subscriptionId), publicationId, args, kwargs);
                } else {
//                    delete _topics[topicUri][subscriptionId];
        ...
        router.emit('Publish', topicUri, this, args, kwargs, ack);
        if (ack) {
            session.send([
                WAMP.PUBLISHED,
                requestId,
