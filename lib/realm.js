...
function Api(router, realm) {
    ...
    // API functions
    // regrpc callback = function(id, args, kwargs)
    ...
    this.resrpc = function (invId, err, args, kwargs) {
        return realm.resrpc(this, invId, err, args, kwargs);
    };
    ...
    this.sendYield = function (callId, err, args, kwargs) {
...
function Realm(router) {
    ...
    this.resrpc = function(session, invId, err, args, kwargs) {
        if (_pending.hasOwnProperty(invId)) {
            var destSession = router.getSession(_pending[invId][1]);
            if (destSession) {
                destSession.sendYield(_pending[invId][0], err, args, kwargs);
            }
        }
