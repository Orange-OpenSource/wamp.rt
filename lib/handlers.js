...
handlers[WAMP.HELLO] = function(args) {
    ...
    if (this.realm === null) {
        ...
        log.trace(realmName+': New session :' + this.sessionId);
...
handlers[WAMP.REGISTER] = function (args) {
  ...
  if (this.checkRealm(WAMP.REGISTER, request))
    this.realm.regrpc(this, request, procUri, options);
};
...
handlers[WAMP.ERROR] = function(msg) {
    ...
    if (requestType === WAMP.INVOCATION) {
        ...
        var invId = requestId;
        realm.resrpc(this, invId, err, args);
    }
}
