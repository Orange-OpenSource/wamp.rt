...
handlers[WAMP.HELLO] = function(args) {
    ...
    if (this.getRealm() === null) {
        ...
        log.trace(realmName+': New session :' + this.getSessionId());
...
handlers[WAMP.REGISTER] = function (args) {
    ...
    if (typeof realm.getrpc(procUri) === 'undefined') {
        ...
        realm.regrpc(procUri, function (invId, args, kwargs) {
...
handlers[WAMP.CALL] = function (args) {
    ...
    var realm = this.getRealm();
    if (!realm.callrpc(procUri, fArgs, kwArgs, resultCallback)) {
        var msg = [
            WAMP.ERROR,
            WAMP.CALL,
            callId,
...
handlers[WAMP.PUBLISH] = function(msg) {
    ...
    var publicationId = realm.publish(topicUri, args, kwargs);
    if (ack) {
        msg = [
            WAMP.PUBLISHED,
            requestId,
