...
handlers[WAMP.SUBSCRIBE] = function(session, args) {
    ...
    var eventCallback = function(publicationId, publishArgs) {
	log.trace('eventCallback', publicationId, publishArgs);
	var msg = [ WAMP.EVENT, subsId, publicationId, {}, publishArgs ];
	session.send(msg);
    };
...
handlers[WAMP.PUBLISH] = function(session, args) {
    ...
    this.publish(topicUri, publicationId, publishArgs);
};
